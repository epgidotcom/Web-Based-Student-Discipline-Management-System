...
form.addEventListener("submit", async function (event) {
  ...
  // If not yet verified, open modal and stop here
  ...
  try {
    ...
    if (!API_BASE) {
      console.warn('[LOGIN] API_BASE is empty. Request will be relative to current origin ->', requestUrl);
    } else {
      console.log('[LOGIN] Fetch:', requestUrl);
    }
  ...
  if (!res.ok) {
    let friendly = data.error || 'Login failed';
    if (res.status === 404) friendly = 'Login API not found (404). Check backend URL configuration.';
    if (res.status === 401) friendly = 'Invalid username/email or password!';
