...
// Diagnostic ping to confirm this router is mounted (GET /api/auth/ping)
router.get('/ping', (req,res) => {
  res.json({ ok: true, router: 'auth', time: new Date().toISOString() });
});
...
router.post('/login', rateLimit({ windowMs: 60_000, max: 25 }), async (req,res) => {
  try {
    ...
    if (process.env.AUTH_DEBUG === 'true') console.log('[AUTH][login] attempt', { username });
    const acct = await findAccount(username);
    if (process.env.AUTH_DEBUG === 'true') console.log('[AUTH][login] accountFound', !!acct);
    if (!acct) return res.status(401).json({ error: 'Invalid credentials' });
    const ok = await bcrypt.compare(password, acct.password_hash);
    if (process.env.AUTH_DEBUG === 'true') console.log('[AUTH][login] passwordMatch', ok);
...
router.post('/forgot-password', rateLimit({ windowMs: 60_000, max: 10 }), async (req,res) => {
  try {
    ...
    const { rows } = await query(`SELECT id FROM accounts WHERE LOWER(email)=LOWER($1) LIMIT 1`, [email]);
    ...
    await query(`INSERT INTO password_resets (account_id, token_hash, token_sha256, expires_at) VALUES ($1,$2,$3,$4)`, [accountId, tokenHash, tokenSha, expiresAt]);
    if (transporter) {
      ...
      try {
        ...
        console.warn('Email send failed, returning token', mailErr);
      }
    }
...
router.post('/reset-password', rateLimit({ windowMs: 60_000, max: 20 }), async (req,res) => {
  try {
    ...
    const { rows } = await query(`SELECT id, account_id FROM password_resets WHERE token_sha256=$1 AND used_at IS NULL AND expires_at > now() ORDER BY created_at DESC LIMIT 1`, [tokenSha]);
